...
export default function NovaEmpresa() {
  ...
  const { id } = useParams();
  const { empresas, adicionarEmpresa, editarEmpresa } = useEmpresas();
  ...
  }, [id, empresas, form, isEdicao]);

  const onFinish = (values) => {
    if (isEdicao) {
      editarEmpresa(parseInt(id), values);
        ...
        <Content style={{ margin: 15, overflowY: 'auto' }}>
          <div style={{ padding: '25px', maxWidth: 1000, margin: '0 auto' }}>
            <Row justify="space-between" align="middle" style={{ marginBottom: 20 }}>
              <Col>
                <Title level={2}>{isEdicao ? 'Editar Empresa' : 'Cadastrar Empresa'}</Title>
              </Col>
              <Col>
